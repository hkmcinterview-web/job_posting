...
def _build_prompt(page: dict, with_images: bool = False) -> str:
    ...
    return intro + (
        ...
        "TABLE_ROW: 표 한 줄, | 구분, 머리글과 개수 일치.\n"
        "          ⚠️ 표는 공고의 '구분(대분류)' 기준으로 한 줄씩 — 공고에 나온 대분류를\n"
        "          하나도 빠짐없이 전부 나열할 것 (최대 10줄. 임의로 합치거나 빼거나 만들지 말 것.\n"
        "          공고에 없는 구분을 지어내는 것 절대 금지). 대분류가 8개면 정확히 8줄.\n"
        "          '직무' 칸에는 그 구분의 세부 직무를 요약해 넣기 — 여러 개면 대표 1~2개 뒤에 '등'\n"
        "          (예: '모터설계 등'). 구분명·직무명·근무지는 원문 표기 그대로 (각 칸 2~12자)\n"
...
def _parse_job(text: str) -> dict:
    ...
    # 표 정합성 — 머리글 개수에 맞게 자르거나 채움 (모든 직무를 담기 위해 최대 10줄)
    ncol = len(job["table_head"])
    if ncol:
        rows = []
        for row in job["table_rows"][:10]:
            row = (row + [""] * ncol)[:ncol]
            rows.append(row)
        job["table_rows"] = rows
    else:
